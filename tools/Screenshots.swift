...
struct ScreenshotExport {
    ...
    static func main() throws {
        ...
        let data = [
            ("terminal", "终端", "terminal", "work", false),
            ("notes", "备忘录", "note.text", "work", false),
            ("calendar", "日历", "calendar", "work", true),
            ("downloads", "下载工具", "arrow.down.circle", "tools", false),
            ("clipboard", "剪贴板", "doc.on.clipboard", "tools", false),
            ("wifi", "Wi-Fi", "wifi", "system", true),
            ("battery", "电池", "battery.100", "system", true),
            ("clock", "时钟", "clock", "system", true)
        ]
        for (id, name, symbol, group, pinned) in data {
            ...
            state.layout.placements[id]?.pinned = pinned
            state.visibility[id] = pinned ? .visible : .hidden
        }
        state.message = "示例数据 · 实际支持情况取决于 macOS 和原应用"
        try render(PocketView(state: state), size: NSSize(width: 1060, height: 800),
                   title: "MenuPocket · 管理与设置", to: output.appendingPathComponent("01-management.png"))
        try render(IconSettingsView(state: state, item: state.items[3]), size: NSSize(width: 480, height: 540),
