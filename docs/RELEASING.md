...
维护者还需在真实桌面验证授权、分组、组入口显示/隐藏、排序、菜单点击及配置迁移。自动化检查不能替代这些交互验收。
...
用户回退应用前备份 `layout.json`，确认旧版本能读取配置格式。0.3.0 使用配置版本 2；回退到 0.2.x 需要使用迁移前的 `layout.json.v1-backup*` 恢复版本 1。更新签名或安装位置可能需要重新授权辅助功能。构建元数据记录提交号，便于定位对应源码。
